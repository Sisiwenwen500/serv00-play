...
installpath="$HOME"
# 引入 utils.sh，这里面包含了支持 small.pl 的核心逻辑
...
# 检查 Sing-box (serv00sb) 进程是否存在
# 注意：即使在 small.pl 上，只要安装脚本没改文件名，进程名依然是 serv00sb
checkHy2Alive() {
...
for obj in "${monitor[@]}"; do
    ...
    if [ "$obj" == "sun-panel" ]; then
        ...
        if ! checkProcAlive "wssh"; then
            # 修正：原代码这里错误调用了 startAlist，改为 startWebSSH
            startWebSSH
            sleep 2
            # 修正：原代码这里错误调用了 checkAlistAlive，改为 checkProcAlive "wssh"
            if ! checkProcAlive "wssh"; then
                msg="webssh 重启失败."
            else
                msg="webssh 重启成功."
